...
function DarkRP.notify(ply, msgtype, len, msg)
    ...
    net.Start("_Notify")
        net.WriteString(msg)
        net.WriteInt(msgtype~=nil and msgtype or 0, 8)
        net.WriteFloat(len)
    net.Send(ply)
end
...
util.AddNetworkString("DarkRP_talktochat")

function DarkRP.talkToChat(receiver, col1, text1, col2, text2, col3, text3, col4, text4, sender)
    if not IsValid(receiver) then return end
    if receiver:IsBot() then return end

    -- Соединяем текст для проверки хуками (видимость)
    local fullText = (text1 or "") .. " " .. (text2 or "") .. " " .. (text3 or "") .. " " .. (text4 or "")
    sender = sender or Entity(0)

    if sender == receiver or hook.Run("PlayerCanSeePlayersChat", fullText, false, receiver, sender) ~= false then
        net.Start("DarkRP_talktochat")
            
            -- БЛОК 1
            col1 = col1 or Color(255, 255, 255)
            net.WriteUInt(col1.r, 8)
            net.WriteUInt(col1.g, 8)
            net.WriteUInt(col1.b, 8)
            net.WriteString(text1 or "")

            -- БЛОК 2
            col2 = col2 or Color(255, 255, 255)
            net.WriteUInt(col2.r, 8)
            net.WriteUInt(col2.g, 8)
            net.WriteUInt(col2.b, 8)
            net.WriteString(text2 or "")
            ...
            -- БЛОК 3
            col3 = col3 or Color(255, 255, 255)
            ...
            -- БЛОК 4 (Новый)
            col4 = col4 or Color(255, 255, 255)
            net.WriteUInt(col4.r, 8)
            net.WriteUInt(col4.g, 8)
            net.WriteUInt(col4.b, 8)
            net.WriteString(text4 or "")

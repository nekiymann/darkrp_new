...
function meta:isLocked()
    ...
    return save and self:isDoor() and save.m_bLocked
end
...
local function SetDoorOwnable(ply)
    ...
    if not IsValid(ent) or not ent:isDoor() or ply:GetPos():DistToSqr(ent:GetPos()) > 40000 then
...
local function OwnDoor(ply)
    ...
    if ent:isKeysOwnedBy(ply) then
        local bAllowed, strReason = hook.Call("playerSellDoor", GAMEMODE, ply, ent)
        ...
        ent:keysUnOwn(ply)
        ent:setKeysTitle(nil)
        local GiveMoneyBack = math.floor((hook.Call("getDoorCost", GAMEMODE, ply, ent) * 0.666) + 0.5)
        ...
        local iCost = hook.Call("getDoorCost", GAMEMODE, ply, ent)
        if not ply:canAfford(iCost) then
            DarkRP.notify(ply, 1, 4, DarkRP.getPhrase("door_cannot_afford"))
            return ""
        end

        local bAllowed, strReason, bSuppress = hook.Call("playerBuyDoor", GAMEMODE, ply, ent)
        if bAllowed == false then
            if strReason and strReason ~= "" then
                DarkRP.notify(ply, 1, 4, strReason)
            end
        ...
        if (ply.OwnedNumz or 0) >= GAMEMODE.Config.maxdoors then
        ...
        ply:addMoney(-iCost)
        if not bSuppress then
            DarkRP.notify(ply, 0, 4, DarkRP.getPhrase("door_bought", DarkRP.formatMoney(iCost), ""))
        end

        ent:keysOwn(ply)
        hook.Call("playerBoughtDoor", GAMEMODE, ply, ent, iCost)
    end
...
local function UnOwnAll(ply, cmd, args)
    ...
    for entIndex, ent in pairs(unownables) do
        local bAllowed, _strReason = hook.Call("playerSellDoor", GAMEMODE, ply, ent)
